...
async def update_post(
    ...
    **Requires:** Authentication (must own the post)
    ...
    try:
        ...
        if request.scheduled_at and post.status == "scheduled":
            scheduler = get_scheduler()
            scheduler.enqueue_scheduled_post(post.id, post.scheduled_at)
...
def _post_to_response(post: Post) -> PostResponse:
    ...
    return PostResponse(
        ...
        title=None,  # Post model uses caption, not title
        description=post.caption,
        hashtags=post.hashtags or [],
        platforms=[post.platform] if post.platform else [],
        platform_status={post.platform: {"status": post.status}} if post.platform else {},
